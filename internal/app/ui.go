...
{{define "home"}}{{template "layout-start" .}}<h1>Home</h1><button class="copy-link" data-copy-link>Copy link</button>{{if .Err}}<p class="error">{{.Err}}</p>{{end}}<div class="cards"><section class="card"><h2>Latest daily note</h2>{{with .Dashboard.LatestDaily}}<a class="biglink" href="{{url .RelPath}}">{{.RelPath}}</a>{{else}}<p>No daily note found.</p>{{end}}</section><section class="card"><h2>Open TODOs</h2><a class="biglink" href="/_todo">Open TODO dashboard</a>{{if .Dashboard.OpenTasks}}<ul class="compact-list">{{range .Dashboard.OpenTasks}}<li><a href="{{.SourceURL}}">{{.Text}}</a>{{if .Due}} <small>📅 {{.Due}}</small>{{end}}</li>{{end}}</ul>{{else}}<p>No open tasks.</p>{{end}}</section><section class="card metric"><h2>Broken links</h2><strong>{{.Dashboard.BrokenLinkCount}}</strong></section><section class="card metric"><h2>Orphan notes</h2><strong>{{.Dashboard.OrphanNoteCount}}</strong></section><section class="card"><h2>Favorites</h2>{{range .Favorites}}<a class="pill" href="{{index . "URL"}}">{{index . "Label"}}</a>{{end}}</section></div><h2>Recent notes</h2><ul class="list">{{range .Recent}}<li><a href="{{url .RelPath}}">{{.RelPath}}</a><small>{{.ModTime.Format "2006-01-02 15:04"}}</small></li>{{end}}</ul>{{template "layout-end" .}}{{end}}
{{define "note"}}{{template "layout-start" .}}<nav class="crumb"><a href="/">Home</a> / {{.Note.RelPath}}</nav><article class="note"><header><h1>{{.Doc.Title}}</h1><button class="copy-link" data-copy-link>Copy link</button></header>{{if .Doc.Tags}}<div class="tag-row">{{range .Doc.Tags}}<a class="tag-badge" href="/_tags/{{.}}">#{{.}}</a>{{end}}</div>{{end}}{{if .Doc.Toc}}<details class="toc" open><summary>Table of contents</summary><ul>{{range .Doc.Toc}}<li class="lvl{{.Level}}"><a href="#{{.ID}}">{{.Text}}</a></li>{{end}}</ul></details>{{end}}<div class="content">{{safe .Doc.HTML}}</div></article><section class="backlinks"><h2>Backlinks</h2>{{if .Backlinks}}<ul>{{range .Backlinks}}<li><a href="{{url .RelPath}}">{{.RelPath}}</a></li>{{end}}</ul>{{else}}<p>No backlinks.</p>{{end}}</section>{{template "layout-end" .}}{{end}}
...
document.addEventListener('DOMContentLoaded', () => { initThemePicker(); initCommandPalette(); restoreSidebarState(); initMobileSidebar(); });
