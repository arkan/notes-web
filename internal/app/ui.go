...
{{define "note"}}{{template "layout-start" .}}<nav class="crumb"><a href="/">Home</a> / {{.Note.RelPath}}</nav><article class="note reading-surface"><header><div><h1>{{.Doc.Title}}</h1>{{if .Doc.Tags}}<div class="tag-row">{{range .Doc.Tags}}<a class="tag-badge chip" href="/_tags/{{.}}">#{{.}}</a>{{end}}</div>{{end}}</div><div class="reading-controls"><label>Font size<select data-font-size-select aria-label="Font size"><option value="normal">Normal</option><option value="small">Small</option><option value="large">Large</option></select></label><button class="focus-toggle btn ghost" data-focus-toggle aria-label="Toggle reading focus">Focus</button><button class="copy-link btn ghost" data-copy-link>Copy link</button></div></header>{{if .Doc.Toc}}<details class="toc" open><summary>Table of contents</summary><ul>{{range .Doc.Toc}}<li class="lvl{{.Level}}"><a href="#{{.ID}}">{{.Text}}</a></li>{{end}}</ul></details>{{end}}<div class="content">{{safe .Doc.HTML}}</div></article><section class="link-panel forward-links"><h2>Forward links <span class="count">{{len .ForwardLinks}}</span></h2>{{if .ForwardLinks}}<ul class="link-context-list">{{range .ForwardLinks}}<li>{{if eq .Kind "unique"}}<a href="{{.URL}}">{{.Display}}</a>{{else}}<span class="missing-link">{{.Display}}</span>{{end}} <small>{{.Kind}}</small></li>{{end}}</ul>{{else}}<p class="empty-state">No forward links.</p>{{end}}</section><section class="link-panel backlinks"><h2>Backlinks <span class="count">{{len .Backlinks}}</span></h2>{{if .Backlinks}}<ul class="link-context-list">{{range .Backlinks}}<li><a href="{{url .Source.RelPath}}">{{.Source.RelPath}}</a><blockquote>{{.Context}}</blockquote></li>{{end}}</ul>{{else}}<p class="empty-state">No backlinks.</p>{{end}}</section>{{template "layout-end" .}}{{end}}
{{define "folder"}}{{template "layout-start" .}}<div class="page-header"><h1>📁 {{.Path}}</h1><button class="copy-link btn ghost" data-copy-link>Copy link</button></div><ul class="list">{{range .Items}}<li><a href="{{index . "URL"}}">{{if index . "Dir"}}📁{{else}}📄{{end}} {{index . "Name"}}</a></li>{{end}}</ul>{{template "layout-end" .}}{{end}}
...
{{define "task-list"}}<ul class="todo-list">{{range .}}<li class="task-row"><span class="task-status" aria-label="{{.StatusLabel}}">{{if .Completed}}✓{{else}}○{{end}}</span><a class="task-title" href="{{.SourceURL}}">{{.Text}}</a><span class="task-meta-row">{{if .Due}}<span class="task-date {{.DateClass}}">Due {{.Due}}</span>{{end}}{{if .Done}}<span class="task-date done-date">Done {{.Done}}</span>{{end}}{{if .ID}}<button class="task-id" data-copy="{{.ID}}" title="Copy task ID">tid:{{.ID}}</button>{{end}}</span></li>{{else}}<li class="empty-state">None.</li>{{end}}</ul>{{end}}
{{define "todo"}}{{template "layout-start" .}}<div class="page-header"><div><p class="eyebrow">Task dashboard</p><h1>TODOs</h1></div></div>{{if .Err}}<p class="error">{{.Err}}</p>{{end}}<p class="muted">Grouped for {{.Today}} from TODO.md files.</p><section class="todo-group overdue"><h2>Overdue <span class="count">{{len .Board.Overdue}}</span></h2>{{template "task-list" .Board.Overdue}}</section><section class="todo-group today"><h2>Today <span class="count">{{len .Board.Today}}</span></h2>{{template "task-list" .Board.Today}}</section><section class="todo-group upcoming"><h2>Upcoming <span class="count">{{len .Board.Upcoming}}</span></h2>{{template "task-list" .Board.Upcoming}}</section><details class="todo-group no-date"><summary><h2>No due date <span class="count">{{len .Board.NoDate}}</span></h2></summary>{{template "task-list" .Board.NoDate}}</details><details class="todo-group done"><summary><h2>Done <span class="count">{{len .Board.Done}}</span></h2></summary>{{template "task-list" .Board.Done}}</details>{{template "layout-end" .}}{{end}}
...
document.addEventListener('DOMContentLoaded', () => { initThemePicker(); initReadingControls(); initCommandPalette(); restoreSidebarState(); initMobileSidebar(); initListFilters(); });
